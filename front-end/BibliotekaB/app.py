    ...
    if session.get('status') == EMPLOYEE:
        ...
        if request.method == 'POST':
            ...
            for i in range(0, int(count)):
                ...
                requests.post('http://10.1.0.121:8002/api/v1/add-book-availability', data=json.dumps(make))
            return search_book()
...
def show_returned_book():
    if session.get('status') == EMPLOYEE:
        data = str('http://10.1.0.121:8002/api/v1/get-check-out/id/') + str(request.args.get('check_id'))
...
def return_book():
    if session.get('status') == EMPLOYEE:
        ...
        requests.post('http://10.1.0.121:8002/api/v1/update-check-out', data=json.dumps(make))
...
def cancel_reservation():
    if session.get('status') == EMPLOYEE:
        data = str('http://10.1.0.121:8002/api/v1/get-reservations/id/') + str(request.args.get('res_id'))
        get_data = requests.get(url=str(data))
        reservation = get_data.json()

        data1 = str('http://10.1.0.121:8002/api/v1/get-check-out/id/') + str(request.args.get('check'))
        ...
        requests.post('http://10.1.0.121:8002/api/v1/update-reservation', data=json.dumps(make))
...
def change_reservation_status():
    if session.get('status') == EMPLOYEE:
        ...
        if request.method == 'POST':
            data = str('http://10.1.0.121:8002/api/v1/get-reservations/id/') + str(request.form.get('res_id'))
            get_data = requests.get(url=str(data))
            reservation = get_data.json()

            data1 = str('http://10.1.0.121:8002/api/v1/get-check-out/id/') + str(request.form.get('check_id'))
            get_check = requests.get(url=str(data1))
            res_details = get_check.json()
...
def view_reservations():
    if session.get('status') == EMPLOYEE:
        ...
        get_check_outs = requests.get('http://10.1.0.121:8002/api/v1/get-check-outs/all')
...
def reserve_book_alt():
    if session.get('status') == EMPLOYEE:
        ...
        get_book_lib = requests.get('http://10.1.0.121:8002/api/v1/get-books-availability/all')
        ...
        if request.method == 'POST':
            ...
            if book_id > 0:
                ...
                get_response = requests.post('http://10.1.0.121:8002/api/v1/add-reservation', data=json.dumps(make_resveration))
                ...
                requests.post('http://10.1.0.121:8002/api/v1/add-check-out', data=json.dumps(make_check_out))
            return view_reservations()
    ...
    if session.get('status') == EMPLOYEE:
        ...
        if request.method == 'POST':
            ...
            if book_id > 0:
                ...
                requests.post('http://10.1.0.121:8002/api/v1/add-check-out', data=json.dumps(make))
            return view_borrowed()
...
def start():
    ...
    for i in get_lib['libraries']:
        if i['id'] == 2:
            ...
            session['library_phone'] = '17 225 22 46'
...
if __name__ == '__main__':
    ...
    ENVIRONMENT_PORT = os.environ.get("APP_PORT", 6060)
